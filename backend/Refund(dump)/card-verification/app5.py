...
def submit_verification_request():
    try:
        ...
        # Use original filename securely
        filename = secure_filename(photo.filename)
        storage_path = f"{datetime.utcnow().timestamp()}_{filename}"

        # Upload directly from photo stream
        res = supabase.storage.from_('refund-photos').upload(
            path=storage_path,
            file=photo.stream,
        ...
        # Get public URL
        public_url = supabase.storage.from_('refund-photos').get_public_url(storage_path)
        ...
        supabase.table('refunds').insert(record).execute()
